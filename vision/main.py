...
def start_process(name, cmd):
    if processes[name] is None or processes[name].poll() is not None:
        print(f"[INFO] Starting process: {cmd}")
        processes[name] = subprocess.Popen(cmd, shell=True)
    else:
        print(f"[WARN] {name} is already running.")
...
def handle_serial_command(cmd):
    cmd = cmd.strip()
    if cmd == "START_PICKING":
        start_process("parse_data", "python3 src/parse_data.py")
    elif cmd == "STOP_PICKING":
        stop_process("parse_data")
    elif cmd == "START_SEARCHING":
        start_process("go_benefits", "python3 src/go_benefits.py")
    elif cmd == "STOP_SEARCHING":
        stop_process("go_benefits")
...
def detect(model: YOLO, frame, ignore_set):
    h, w = frame.shape[:2]
    mid_x = w // 2
    best_for = {"top": {"label": None, "conf": -1.0, "bbox": None},
                "bottom": {"label": None, "conf": -1.0, "bbox": None}}
...
def print_status(matrix, last_seen_ts, now, infer_ms):
    ...
    print(
        f"{matrix} | last seen → top: {format_time('top')}, bottom: {format_time('bottom')} | "
...
def annotate_and_show(r, frame, matrix, mid_x, best_for):
    ...
    for side, color in (("top", (0, 255, 0)), ("bottom", (0, 200, 255))):
        ...
        y_txt = max(0, y1 - 8)
        cv2.putText(annotated, label_text, (x1, y_txt),
